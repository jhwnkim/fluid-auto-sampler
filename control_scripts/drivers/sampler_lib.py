...
class acton_trigger():
    def __init__(self, address='COM3', timeout=30.0, verbose=True):
...
#         self.uno.timeout = 20
    ...
    def send_trigger(self, N=10, pm=None):
        """
        Sends given number of triggers to acton controller
        Parameters:
            - N: integer (number of triggers)
        ...
        for i in range(0, N):
            ...
            if state == b'1':
                t_0=time()
                state = self.send_single()

                t=time()-t_0
                while state ==b'2' and t-t_0 < self.timeout: # Acton is acquiring spectra and system has not timed out
                    state = self.get_state()
                    # If power meter is available log power measurements
                    if pm is not None:
                        if pm.measPower(byref(power)) == 0: # if power measurement is successful
                            power_measurements.append(float(power.value))
                ...
                if i==0: # State is in idle even before the first acquisition
                    print('Acton state at 0, please check WinSpec and begin acquisition')
                else: # State is in idle
...
class sampler():
    def __init__(self, verbose=False):
        ...
        # Variables
        self.verbose = verbose
        self.wavelength = 830.0 # nm, excitation wavelength
        ...
        try:
            self.acton = acton_trigger(address='COM3', timeout=20.0, verbose=False)
        except:
        ...
        try:
            ...
            print('Thorlabs Powermeter connected. Setting wavelength to {} nm'.format(self.wavelength))
            self.pm.setWavelength(c_double(self.wavelength)) # Set wavelength
            self.pm.setPowerUnit(c_int16(0)) # Set units to Watt

################## Fluidics related methods
...
################## power meter related methods
    def get_power(self, timeout=10.0):
        """
        Method to get power from laser tap power meter
        return:
            float of power in
        """
        power = c_double()
        if self.pm is not None:
            ...
            t = time()-t_0
            while t<timeout:
                if self.pm.measPower(byref(power)) == 0: # if power measurement is successful
